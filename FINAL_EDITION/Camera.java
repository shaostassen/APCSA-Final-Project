...
{ //Camera start
	...
	{ //If the key is pressed start
		...
		if((key.getKeyCode() == KeyEvent.VK_UP)) //If a up is pressed,
			forward = true; //Move forward
		if((key.getKeyCode() == KeyEvent.VK_DOWN)) //If a down is pressed,
	...
	{ //Key release start
		...
		if((key.getKeyCode() == KeyEvent.VK_UP)) //If up was released
			forward = false; //Stop moving up
		if((key.getKeyCode() == KeyEvent.VK_DOWN)) //If down was released
