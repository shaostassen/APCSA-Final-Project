...
public class Screen {
	...
	public Screen(int[][] m, int mapW, int mapH, ArrayList<Texture> tex, int w, int h, Monster monster) {
		map = m;
		mapWidth = mapW;
		mapHeight = mapH;
		textures = tex;
	...
	public void/*int[]*/ update(Camera camera, int[] pixels) {
	    ...
	    for (Sprite sprite: sprites) {
	    	...
	    	if (sprite instanceof Gold) {
	    		spriteX = sprite.getX() - camera.xPos;
	    		spriteY = sprite.getY() - camera.yPos;
	    	}
	    	else {
	    		spriteX = ((Monster) sprite).getTrueX() - camera.xPos;
	    		spriteY = ((Monster) sprite).getTrueY() - camera.yPos;
	    	}
	        ...
	        if (sprite instanceof Gold) { texWidth = 880; texHeight = 522; }
	        	...
	        	if (transformY >= 0 && stripe >= 0 && stripe <= w && transformY <= ZBuffer[stripe]) {
	        		for(int y = drawStartY; y < drawEndY; y++) {
	        			 ...
	        			 try {
	        				 ...
	        				 else { color = monster.curTexture.pixels[texX][texY]; }
